...
\begin{figure}
	\centering
	\includegraphics[width=5in]{Figures/ch4_St005_lambda_evolution_v2.png}
	\caption{Evolution of the independent vortex ring ($St_{DF}=0.05$), as visualized using swirling strength. Images are shown at phase steps of roughly $\pi/8$, starting with a phase of $\pi/4$.}
...
An acceleration of the less-coherent structures can also be observed in \fig{fig:ch4_impulse_structure_disintegration}, as the small eddies are located further downstream in the final frames than they would be if following a constanst convective velocity (as denoted by the red line overlain on the frames).
...
Therefore, the twin effects of rapid disintegration (\ie rapid envelope modulation) and acceleration would to greatly enhance the acoustic efficiency from the vortex, and potentially explain why the region in which the turbulent structure rapidly decays appears to dominate over the region in which the turbulent structure rapidly grows in terms of the noise emission.
\begin{figure}
	\centering
	\includegraphics[width=3.5in]{Figures/ch4_St005_centerline_mach_temporal.png}
	\caption{Fluctuations associated with the passage of large-scale structures in the axial velocity along the jet centerline at $x/D = 4$.}
